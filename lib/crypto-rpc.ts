...
export async function cryptoRpcUrl(chainId: number) {
  ...
  const publicRpc: Record<number, string> = {
    1: "https://ethereum-rpc.publicnode.com",
    56: "https://bsc-rpc.publicnode.com",
    137: "https://polygon-bor-rpc.publicnode.com",
    8453: "https://base-rpc.publicnode.com",
  };
  return values[`CRYPTO_RPC_URL_${chainId}`] || values.CRYPTO_RPC_URL || publicRpc[chainId] || "";
}

export async function cryptoRpc(url: string, method: string, params: unknown[]) {
  ...
  if (!response.ok || payload?.error || payload?.result == null) throw new Error(payload?.error?.message || "RPC_UNAVAILABLE");
