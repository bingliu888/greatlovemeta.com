...
export async function smartPay5ReceiptByTransactionId(input: {
  ...
  if (input.transactionHash) {
    const receipt = await rpc<{ transactionHash?: string; status?: string; blockNumber?: string; logs?: { address?: string; topics?: string[]; data?: string }[] }>("eth_getTransactionReceipt", [input.transactionHash]);
    const receiptTransactionId = smartPay5TransactionIdFromReceipt(receipt.logs || [], input.contract);
    if (!receipt.transactionHash || receipt.transactionHash.toLowerCase() !== input.transactionHash.toLowerCase() || receipt.status !== "0x1" || !receipt.blockNumber || receiptTransactionId?.toLowerCase() !== input.transactionId.toLowerCase()) throw new Error("TRANSACTION_RECEIPT_INVALID");
    return receipt;
  }
