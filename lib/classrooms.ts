...
export async function directoryClasses(view: "public" | "trial" | "private" | "mine" | "joined_public" | "joined_trial" | "joined_private", user: SessionUser | null) {
  ...
  if (view === "mine") {
    ...
    return (await db.prepare(`${selection} LEFT JOIN class_cohosts c ON c.room_id=r.id WHERE (r.host_user_id=? OR c.user_id=?) AND r.status='active' GROUP BY r.id ORDER BY r.updated_at DESC LIMIT 100`).bind(user.id,user.id).run<ClassRoom>()).results || [];
  }
  if (view.startsWith("joined_")) {
    if (!user) return [];
    const classType=view.slice(7) as ClassType;
    return (await db.prepare(`${selection} JOIN class_join_history h ON h.room_id=r.id
      WHERE h.user_id=? AND r.class_type=? AND r.status='active' AND r.host_user_id<>?
      AND NOT EXISTS(SELECT 1 FROM class_cohosts c WHERE c.room_id=r.id AND c.user_id=?)
      ORDER BY h.last_joined_at DESC LIMIT 100`).bind(user.id,classType,user.id,user.id).run<ClassRoom>()).results || [];
  }
  if (view === "private") {
    if (!user) return [];
    return (await db.prepare(`${selection} JOIN class_invites i ON i.room_id=r.id WHERE r.class_type='private' AND r.status='active' AND (lower(i.email)=lower(?) OR r.host_user_id=?) ORDER BY r.updated_at DESC LIMIT 100`).bind(user.email,user.id).run<ClassRoom>()).results || [];
  }
...
export async function createClassRoom(user: SessionUser, input: Record<string,unknown>) {
  ...
  const trialMinutes=classType==="trial"?1440:0;
  ...
  await db.prepare(`INSERT INTO class_rooms(id,code,host_user_id,host_email,host_name,title,description,subject,class_type,streaming_mode,realtime_mode,starts_at,duration_minutes,trial_minutes,tuition_cents,password_hash,created_at,updated_at)
  ...
  for(const email of new Set(invites))await db.prepare("INSERT OR IGNORE INTO class_invites(id,room_id,email,created_at) VALUES(?,?,?,?)").bind(createId(),id,email,now).run();
  return {id,code};
}

export async function recordClassJoin(userId:string,roomId:string,joinedAt=Math.floor(Date.now()/1000)){
  await getDatabase().prepare(`INSERT INTO class_join_history(user_id,room_id,first_joined_at,last_joined_at)
    VALUES(?,?,?,?) ON CONFLICT(user_id,room_id) DO UPDATE SET last_joined_at=excluded.last_joined_at`).bind(userId,roomId,joinedAt,joinedAt).run();
}
...
export async function classAccess(room: ClassRoom, user: SessionUser | null, startTrial=false) {
  ...
  const manager=await canManageClass(room,user);
  if(manager)return {allowed:true,admin,host,manager:true,status:"teacher" as const};
  if(room.classType!=="private"){
    const paid=await paidClassAccess(room,user,startTrial);
    return {...paid,admin,host,manager:false};
  }
  if(!user)return {allowed:false,admin:false,host:false,manager:false};
  const invited=await getDatabase().prepare("SELECT id FROM class_invites WHERE room_id=? AND lower(email)=lower(?) LIMIT 1").bind(room.id,user.email).first();
