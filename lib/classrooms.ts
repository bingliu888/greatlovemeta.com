...
export async function createClassRoom(user: SessionUser, input: Record<string,unknown>) {
  ...
  await db.prepare(`INSERT INTO class_rooms(id,code,host_user_id,host_email,host_name,title,description,subject,class_type,streaming_mode,starts_at,duration_minutes,trial_minutes,tuition_cents,password_hash,created_at,updated_at)
