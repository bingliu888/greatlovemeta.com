...
export function walletRpcErrorCode(error: unknown) { const pending: unknown[] = [error]; const seen = new Set<unknown>(); while (pending.length) { const value = pending.shift(); if (!value || seen.has(value)) continue; seen.add(value); if (typeof value !== "object") continue; const record = value as Record<string, unknown>; const code = Number(record.code); if (Number.isFinite(code)) return code; for (const key of ["data", "error", "cause", "originalError"]) if (record[key] != null) pending.push(record[key]); } return null; }
