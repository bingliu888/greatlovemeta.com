const EXPORT_FORMAT = "greatlovemeta.com/cloudflare-migration";
const EXPORT_VERSION = 1;
const MAX_LIMIT = 500;
const MAX_OBJECT_CHUNK = 4 * 1024 * 1024;
const INTERNAL_TABLE_PREFIXES = ["sqlite_", "_cf_", "__new_"];
const INTERNAL_TABLES = new Set(["d1_migrations", "__drizzle_migrations", "__appgarden_migrations"]);

type MigrationBindings = { DB?: D1Database; BUCKET?: R2Bucket };

async function bindings(): Promise<MigrationBindings> {
  const { env } = await import("cloudflare:workers");
  return env as MigrationBindings;
}
...
async function inventory(db: D1Database, bucket?: R2Bucket) {
  ...
  return { tables: counts, totalRows, r2: { enabled: Boolean(bucket), totalObjects, totalBytes } };
}

async function d1Page(db: D1Database, url: URL) {
  const table = url.searchParams.get("table") ?? "";
  if (!isBusinessTable(table) || !(await tableNames(db)).includes(table)) return json({ error: "Unknown migration table" }, 400);
  const after = safeInteger(url.searchParams.get("after"), 0);
  const limit = safeInteger(url.searchParams.get("limit"), 250, MAX_LIMIT);
  const result = await db.prepare(`SELECT rowid AS __migration_rowid__, * FROM "${table}" WHERE rowid > ? ORDER BY rowid LIMIT ?`).bind(after, limit).all<Record<string, unknown>>();
...
async function r2List(bucket: R2Bucket, url: URL) {
  ...
  if (cursor && (cursor.length > 4096 || /[\u0000-\u001f\u007f]/.test(cursor))) return json({ error: "Invalid R2 cursor" }, 400);
  const page = await bucket.list({ cursor, limit: 250, include: ["httpMetadata", "customMetadata"] });
  return json({
    format: EXPORT_FORMAT,
    version: EXPORT_VERSION,
    mode: "r2-list",
    objects: page.objects.map((object) => ({ key: object.key, size: object.size, etag: object.etag, uploaded: object.uploaded.toISOString(), httpMetadata: object.httpMetadata ?? null, customMetadata: object.customMetadata ?? null })),
...
async function r2Object(bucket: R2Bucket, url: URL) {
  ...
  if (!key || key.length > 1024 || /[\u0000-\u001f\u007f]/.test(key)) return json({ error: "Invalid R2 key" }, 400);
  ...
  return json({ format: EXPORT_FORMAT, version: EXPORT_VERSION, mode: "r2-object", key, offset, bytes: bytes.length, data: btoa(binary) });
}
...
export async function cloudflareMigrationExport(request: Request) {
  ...
  if (!DB) return json({ error: "Migration database binding unavailable" }, 503);
  try {
    const url = new URL(request.url);
    const mode = url.searchParams.get("mode") ?? "inventory";
    if (mode === "inventory") return json({ format: EXPORT_FORMAT, version: EXPORT_VERSION, mode, ...(await inventory(DB, BUCKET)) });
    if (mode === "d1") return d1Page(DB, url);
    if (mode === "r2-list") return BUCKET ? r2List(BUCKET, url) : json({ format: EXPORT_FORMAT, version: EXPORT_VERSION, mode, objects: [], nextCursor: null });
    if (mode === "r2-object") return BUCKET ? r2Object(BUCKET, url) : json({ error: "R2 binding unavailable" }, 404);
