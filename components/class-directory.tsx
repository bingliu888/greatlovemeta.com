...
type Room = { id:string; code:string; title:string; description:string; subject:string; classType:"public"|"trial"|"private"; streamingMode:"audio"|"video"; startsAt:number; hostName:string; streamActive:number; tuitionCents:number; isHelpRoom:number };
...
export function ClassDirectory({ lang, initialView = "public" }: { lang: import("../lib/site-locale").SiteLanguage; initialView?:string }) {
  ...
  const cards=(items:Room[])=>items.length?<section className="class-card-list">{items.map(room=><article className="class-card" key={room.id}><div><p>{room.isHelpRoom?helpUiFor(lang).label:room.subject||(zh?"社区课程":"COMMUNITY COURSE")}</p><h2>{room.isHelpRoom?helpUiFor(lang).label:room.title}</h2>{room.isHelpRoom?null:<span>{room.description||(zh?"教师尚未添加课程介绍。":"The Teacher has not added a description.")}</span>}<small>{new Date(room.startsAt*1000).toLocaleString(zh?"zh-CN":"en-US",{timeZone:"America/Los_Angeles"})} · {room.hostName} · {room.streamingMode==="audio"?(zh?"音频":"Audio"):(zh?"音视频":"A/V")}</small></div><div><i className={room.streamActive?"live":""}>{room.streamActive?"LIVE":"READY"}</i>{room.classType==="trial"&&<b>$${(room.tuitionCents/100).toFixed(2)}</b>}<a href={`/${lang}/classes/${room.code}`}>{zh?"进入":"Enter"} →</a></div></article>)}</section>:<div className="class-empty compact"><h2>{zh?"这里暂时还没有课程":"No courses here yet"}</h2><p>{zh?"请选择其他课程类型或稍后再来。":"Check another course type or return later."}</p></div>;
