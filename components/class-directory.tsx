...
export function ClassDirectory({ lang, initialView = "public" }: { lang:"en"|"zh"; initialView?:string }) {
  ...
  return <main className="class-directory-page">
    <section className="class-directory-hero"><p>{view==="mine"?(zh?"我的课程":"MY COURSES"):(zh?"社区课程":"COMMUNITY COURSES")}</p><h1>{view==="mine"?(zh?"创建、管理与加入的课程":"Courses you create, manage, and join"):(zh?"与社区一起学习":"Learn with the community")}</h1><span>{view==="mine"?(quota.max===null?(zh?"平台管理员可无限创建课程。":"Platform administrators can create unlimited classes."):(zh?`教师课程额度：${quota.used} / ${quota.max}`:`Teacher course quota: ${quota.used} / ${quota.max}`)):(zh?"进入公课、先试课再加入，或通过邀请参加私课。":"Enter a public course, try a course, or join a private course by invitation.")}</span>{canCreate&&<Link className="class-create-link" href={`/${lang}/classes/create`}>{zh?"创建课程":"Create course"}</Link>}</section>
    <nav className="class-tabs" aria-label={zh?"课程目录":"Course directory"}><button className={view==="public"?"active":""} onClick={()=>setView("public")}>{zh?"公课":"Public"}</button><button className={view==="trial"?"active":""} onClick={()=>setView("trial")}>{zh?"推荐课程":"Referred course"}</button><button className={view==="private"?"active":""} onClick={()=>setView("private")}>{zh?"私课":"Private"}</button><button className={view==="mine"?"active":""} onClick={()=>setView("mine")}>{zh?"我的课程":"My courses"}</button></nav>
    {loading?<div className="class-empty">{zh?"正在读取课程…":"Loading courses…"}</div>:view==="mine"?<><section className="my-class-section"><h2>{zh?"我创建或管理的课程":"Courses I create or manage"}</h2><p>{zh?"教师与联合教师可以在这里管理课程。":"Teachers and Co-teachers can manage courses here."}</p>{cards(rooms)}</section><section className="my-class-section"><h2>{zh?"我加入的课程":"Courses I joined"}</h2><nav className="class-tabs joined-tabs"><button className={joinedView==="public"?"active":""} onClick={()=>setJoinedView("public")}>{zh?"公课":"Public"}</button><button className={joinedView==="trial"?"active":""} onClick={()=>setJoinedView("trial")}>{zh?"推荐课程":"Referred course"}</button><button className={joinedView==="private"?"active":""} onClick={()=>setJoinedView("private")}>{zh?"私课":"Private"}</button></nav>{cards(joinedRooms)}</section></>:cards(rooms)}
  </main>;
}
