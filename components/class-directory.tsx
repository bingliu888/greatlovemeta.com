...
export function ClassDirectory({ lang, initialView = "public" }: { lang:"en"|"zh"; initialView?:string }) {
  ...
  return <main className="class-directory-page">
    <section className="class-directory-hero"><p>{view==="mine"?(zh?"我的课程":"MY CLASSES"):(zh?"社区课堂":"COMMUNITY CLASSES")}</p><h1>{view==="mine"?(zh?"创建、管理与加入的课程":"Classes you create, manage, and join"):(zh?"与社区一起学习":"Learn with the community")}</h1><span>{view==="mine"?(quota.max===null?(zh?"平台管理员可无限创建课程。":"Platform administrators can create unlimited classes."):(zh?`教师课程额度：${quota.used} / ${quota.max}`:`Teacher class quota: ${quota.used} / ${quota.max}`)):(zh?"进入公课、先试课再加入，或通过邀请参加私课。":"Enter a public class, try a course, or join a private class by invitation.")}</span>{canCreate&&<Link className="class-create-link" href={`/${lang}/classes/create`}>{zh?"创建课程":"Create class"}</Link>}</section>
