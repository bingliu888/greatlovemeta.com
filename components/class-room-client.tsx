...
function ConnectedRoom({client,room,identity,manager,displayName,role,mic,camera,onMedia,onLeave}:{client:RTKClient;room:Room;identity:string;manager:boolean;displayName:string;role:Role;mic:boolean;camera:boolean;onMedia:(mic:boolean,camera:boolean)=>Promise<void>;onLeave:()=>void}){
  ...
  const load=useCallback(async()=>{const[m,c]=await Promise.all([fetch(`/api/classes/${room.code}/media`,{cache:"no-store"}),fetch(`/api/classes/${room.code}/chat`,{cache:"no-store"})]);if(m.ok)setMedia(await m.json());if(c.ok)setMessages((await c.json()).messages||[]);await fetch(`/api/classes/${room.code}/media`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({action:"heartbeat",identity})});},[identity,room.code]);
  ...
  async function send(event:React.FormEvent){event.preventDefault();if(!body.trim())return;const response=await fetch(`/api/classes/${room.code}/chat`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({body})});if(response.ok){setBody("");await load()}else setError("Sign in as a member to send messages.")}
  return <>
    <header className="class-room-controls"><div><i className="live"/><b>Live classroom</b><small>{role==="viewer"?"Viewer":media?.streamingMode==="audio"?"Audio":"Audio / Video"}</small></div><nav>
      ...
      {manager&&room.classType!=="private"&&<button className={!media?.muteAll?"on":""} onClick={async()=>{await fetch(`/api/classes/${room.code}/media`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({action:"mute-all",mic:!media?.muteAll})});await load()}}>{media?.muteAll?"Open media":"Mute all"}</button>}
...
export function ClassRoomClient({room,displayName,manager,lang}:{room:Room;displayName:string;manager:boolean;lang:"en"|"zh"}){
  ...
  if(!joined||!client)return <section className="class-waiting"><span className="stream-spinner"/><h2>{manager?"Start the live classroom":"Waiting for live classroom"}</h2><p>{manager?"Start an independent RealtimeKit session. Microphone and camera remain off until selected.":"You will join automatically as a viewer when streaming starts. No device permission is requested."}</p>{manager&&<button onClick={()=>void connect({start:true})}>Start live streaming</button>}{error&&<p role="alert">{error}</p>}</section>;
