...
export function SmartPayAdminConsole({
  ...
  useEffect(() => {
    ...
    void fetch(`/api/admin/crypto-payments/rules?chainId=${selectedChainId}`, { cache: "no-store" })
      ...
      .then((data: { enabledPresetKeys?: string[]; knownPresetKeys?: string[]; stalePresetKeys?: string[] }) => {
        if (cancelled) return;
        setEnabledPresetKeys(new Set(data.enabledPresetKeys || []));
        setKnownPresetKeys(new Set(data.knownPresetKeys || []));
        setStalePresetKeys(new Set(data.stalePresetKeys || []));
        setConfirmationStateChainId(selectedChainId);
      })
      .catch(() => {
        if (cancelled) return;
        setEnabledPresetKeys(new Set());
        setKnownPresetKeys(new Set());
        setStalePresetKeys(new Set());
        setConfirmationStateChainId(null);
        setMessage(zh ? "无法读取付款项目确认状态；停用与确认操作已暂时锁定。" : "Payment-item confirmation state is unavailable; Stop and confirmation actions are temporarily locked.");
      });
  ...
  async function connectWallet() {
    ...
    try {
      const result = await connectEvmWallet({
        ...
        appName: GREATLOVEMETA_WALLET_CONNECT.admin.appName,
        description: GREATLOVEMETA_WALLET_CONNECT.admin.description,
        ...
        setMessage(sourceSubmitted
          ? sourceResult.explorerVerified
            ...
            : /ETHERSCAN_API_KEY is not configured/i.test(sourceResult.explorerMessage || "")
              ? (zh ? `${contractName} 已部署并由 Sourcify 精确验证；PolygonScan 需先为生产 Worker 配置 ETHERSCAN_API_KEY，之后可在本页重试：` + shortAddress(deployedAddress) : `${contractName} deployed with an exact Sourcify match. Configure ETHERSCAN_API_KEY in the production Worker, then retry PolygonScan from this page: ` + shortAddress(deployedAddress))
    ...
    try {
      ...
      setMessage(result.explorerVerified
        ...
        : /ETHERSCAN_API_KEY is not configured/i.test(result.explorerMessage || "")
          ? (zh ? "Sourcify 已精确验证；PolygonScan 尚未提交，因为生产 Worker 未配置 ETHERSCAN_API_KEY。配置密钥后可再次点击此按钮，不需要重新部署或支付 Gas。" : "Sourcify is an exact match. PolygonScan was not submitted because ETHERSCAN_API_KEY is not configured in the production Worker. Retry after configuring it; no redeployment or gas is required.")
  ...
  async function sendOwnerTransaction(functionName: string, args: readonly unknown[], key: string, prompt: string) {
    ...
    try {
      ...
      const reason = error instanceof Error ? error.message : "";
      setMessage(reason === "NOT_OWNER" ? (zh ? "您连接的钱包不是 SmartPay5 Owner。" : "Your connected wallet is not the SmartPay5 owner.")
  ...
  async function confirmSmartPay5Preset(preset: SmartPay5RulePreset) {
    if (enabledPresetKeys.has(preset.key)) {
      setMessage(zh ? "请先点击停用；停用只会从本站付款选择中隐藏该项目，不会调用钱包或合约。" : "Click Stop first. Stop only hides this item from this site's payment selection; it does not call the wallet or contract.");
      return;
    }
    ...
    try {
      ...
      setEnabledPresetKeys(current => new Set(current).add(preset.key));
      setKnownPresetKeys(current => new Set(current).add(preset.key));
      ...
      setMessage(zh ? "链上交易已确认，但服务器复核未完成；此项目仍不会出现在付款选择中，请再次点击重新确认。" : "The on-chain transaction confirmed, but server verification did not finish. This item remains hidden from payment selection; click Re-confirm again.");
    }
  }
  ...
  async function stopSmartPay5Preset(preset: SmartPay5RulePreset) {
    ...
    try {
      ...
      setEnabledPresetKeys(current => { const next = new Set(current); next.delete(preset.key); return next; });
      setKnownPresetKeys(current => new Set(current).add(preset.key));
      setStalePresetKeys(current => { const next = new Set(current); next.delete(preset.key); return next; });
      setMessage(zh ? "已停用：此项目不会出现在付款选择中。点击重新确认才会再次调用合约并恢复。" : "Stopped: this item is hidden from payment selection. Only Re-confirm will call the contract and restore it.");
    } catch {
      setMessage(zh ? "停用未完成；付款项目状态没有改变。" : "Stop did not complete; the payment-item state was not changed.");
    } finally {
      setBusy("");
    }
  ...
  async function syncMember(member: MatchedMember, records: TransactionRecord[]) {
    ...
    const eligible = records.filter(record => smartPayTransactionNeedsReconciliation(record, member.payerId, record.productOwnerRefId));
  ...
  async function loadLatestTransactions() {
    ...
    if (payerId && !/^[A-HJ-NP-Z2-9]{6}$/.test(payerId)) throw new Error(zh ? "请输入有效的六位付款人 ID，或留空读取最新 25 条。" : "Enter a valid six-character PayerID, or leave it blank for the latest 25.");
  ...
  async function refreshTransactions(autoSync = true) {
    ...
    try {
      ...
      setMessage(syncErrors.length
        ...
        : ambiguous ? (zh ? "交易已读取；有付款人 ID 匹配多个历史会员别名，请手动选择会员同步。" : "Transactions loaded. A PayerID matches multiple historical member aliases, so choose the member manually.")
  ...
  return <div className="smartpay-console-grid">
    <section className="smartpay-console-card smartpay-contract-card">
      ...
      {verificationLinks && !verificationLinks.explorerVerified ? <p className="smartpay-source-warning">{/ETHERSCAN_API_KEY is not configured/i.test(verificationLinks.explorerMessage || "")
        ? (zh ? "原因：生产 Worker 尚未配置 ETHERSCAN_API_KEY。Sourcify 精确验证仍然有效；配置密钥后可在此重试 PolygonScan，不需要重新部署或支付 Gas。" : "Reason: ETHERSCAN_API_KEY is not configured in the production Worker. The Sourcify exact match remains valid; retry PolygonScan here after configuration without redeployment or gas.")
    ...
    <section className="smartpay-console-card smartpay-wide-card">
      ...
      <p>{zh ? "SmartPay5 的 USDT 项目在链上保存 100% USDT、100% 等值 GLC 与 10 亿 GLC 门槛，实际 USDT 比例保存在数据库；所有非 USDT 项目按后台金额写为 100% 单币规则。" : "SmartPay5 stores full USDT/GLC equivalents and the 1-billion-GLC threshold for USDT items, while the active USDT ratio stays in the database. Every non-USDT item is stored as a 100% single-token rule."}</p>
      {smartPay5PresetRows.length ? <div className="smartpay-rule-list smartpay-preset-list">{smartPay5PresetRows.map(({ preset, status }) => {
        ...
        const statusLabel = databaseState === "stopped" ? (zh ? "待重新确认 · 付款不可选" : "Awaiting Re-confirm · unavailable") : databaseState === "stale" ? (zh ? "数据库价格已变 · 需重新确认 · 付款不可选" : "Database price changed · Re-confirm required · unavailable") : databaseState === "unconfirmed" ? (zh ? "尚未确认 · 付款不可选" : "Not confirmed · unavailable") : status.state === "missing" ? (zh ? "数据库已启用 · 链上缺失" : "Database enabled · missing on-chain") : null;
        const stopLabel = status.state === "different" ? (zh ? "价格已改变 · 停用" : "Price changed · Stop") : status.state === "configured" ? (zh ? "已确认 · 停用" : "Confirmed · Stop") : (zh ? "数据库已启用 · 停用" : "Database enabled · Stop");
        return <article key={preset.key}><div><strong>{preset.months} {zh ? "个月" : "months"} · {preset.mode === "dual" ? `${preset.primaryTokenSymbol} / ${preset.secondaryTokenSymbol}` : preset.primaryTokenSymbol}</strong><small>{preset.mainId} / {preset.secondId || "—"}</small></div><div><b>{preset.mode === "dual" ? `${preset.primaryTokenAmount} ${preset.primaryTokenSymbol} ↔ ${preset.secondaryTokenAmount} ${preset.secondaryTokenSymbol}` : `${preset.primaryTokenAmount} ${preset.primaryTokenSymbol}`}</b><small>{preset.mode === "dual" ? `${zh ? `当前本站：${preset.primaryPercent}% / ${preset.secondaryPercent}%` : `Current site: ${preset.primaryPercent}% / ${preset.secondaryPercent}%`} · ${zh ? `门槛 ${preset.minimumSecondaryBalance} GLC` : `Threshold ${preset.minimumSecondaryBalance} GLC`}` : (zh ? "非 USDT · 100% 单币付款" : "Non-USDT · 100% single-token payment")}</small></div>{control.showStop ? <button type="button" className={status.state === "configured" ? "rule-enabled" : "rule-disabled"} title={zh ? "仅从本站付款选择中隐藏；不调用钱包或合约" : "Hide only from this site's payment selection; no wallet or contract call"} onClick={() => void stopSmartPay5Preset(preset)} disabled={!confirmationStateReady || Boolean(busy)}>{busy === stopKey ? "…" : stopLabel}</button> : statusLabel ? <span className="rule-disabled">{statusLabel}</span> : null}{control.showConfirm ? <button type="button" className="button primary" onClick={() => void confirmSmartPay5Preset(preset)} disabled={!confirmationStateReady || !contractState || Boolean(busy)}>{busy === key ? "…" : control.confirmKind === "reconfirm" ? (zh ? "重新确认" : "Re-confirm") : (zh ? "确认写入" : "Confirm on-chain")}</button> : null}</article>;
    ...
    <section className="smartpay-console-card smartpay-wide-card">
      ...
      <p>{zh ? "输入付款人 ID 读取该账户最新 100 条交易；留空读取合约最新 25 条。只有付款人 ID、产品所有者 RefID 与套餐匹配且尚未入账的订阅才会自动更新；付款钱包只作审计展示。" : "Enter a PayerID to read that account's latest 100 transactions, or leave it blank for the latest 25 contract transactions. Only unclaimed subscriptions matching the PayerID, product-owner RefID, and package are synchronized; the funding wallet is audit-only."}</p>
      <div className="smartpay-transaction-query"><label><span>{zh ? "付款人 ID（可留空）" : "PayerID (optional)"}</span><input value={queryPayerId} onChange={event => setQueryPayerId(event.target.value.trim().toUpperCase())} placeholder="ABC234" maxLength={6} disabled={!transactionReadReady}/></label><button type="button" className="button primary" onClick={() => void refreshTransactions()} disabled={!contractConfigured || !transactionReadReady || Boolean(busy)}>{busy === "transactions" ? "…" : (zh ? "刷新交易并核对订阅" : "Refresh transactions & subscriptions")}</button></div>
      <p className="smartpay-record-count">{!transactionReadReady ? (zh ? "交易读取：等待重新部署" : "Transaction reads: redeploy required") : queryPayerId ? (zh ? `付款人 ID 交易：${transactions.length} / ${transactionTotal}` : `PayerID transactions: ${transactions.length} of ${transactionTotal}`) : (zh ? `最新交易：${transactions.length} / 合约总计 ${transactionTotal}` : `Latest transactions: ${transactions.length} of ${transactionTotal} total`)}</p>
      {transactions.length ? <div className="smartpay-transaction-table" role="region" aria-label={`${contractName} transactions`}><table><thead><tr><th>TransactionID</th><th>{zh ? "时间 / 付款人 ID / 产品所有者 RefID" : "Time / PayerID / Owner RefID"}</th><th>mainID / secondID</th><th>{zh ? "代币 / 数量" : "Token / amount"}</th><th>{zh ? "订阅状态" : "Subscription"}</th></tr></thead><tbody>{transactions.map(record => <tr key={record.transactionId}><td><code>{record.transactionId}</code></td><td>{new Date(record.timestamp * 1000).toLocaleString(locale)}<small>{zh ? "付款人 ID" : "PayerID"} · {record.payerId}</small><small>{zh ? "产品所有者 RefID" : "Owner RefID"} · {record.refId}</small><small>{zh ? "付款钱包" : "Funding wallet"} · {record.wallet}</small></td><td>{record.mainId}<small>{record.secondId || "—"}</small></td><td>{record.primaryTokenAmount}<small>{record.primaryTokenAddress}</small>{hasSecondaryTransaction(record) ? <>{record.secondaryTokenAmount}<small>{record.secondaryTokenAddress}</small></> : null}</td><td><span className={record.subscriptionRecorded ? "rule-enabled" : "rule-disabled"}>{record.subscriptionRecorded ? (zh ? "已确认" : "Confirmed") : (zh ? "待核对" : "Pending")}</span>{record.subscriptionEndsAt ? <small>{zh ? `订阅至 ${subscriptionEndLabel(record.subscriptionEndsAt, locale)}` : `Subscription through ${subscriptionEndLabel(record.subscriptionEndsAt, locale)}`}</small> : null}</td></tr>)}</tbody></table></div> : null}
      {matchedMembers.length ? <div className="smartpay-matched-members"><strong>{zh ? "匹配会员" : "Matched members"}</strong>{matchedMembers.map(member => {
        const needsReconciliation = transactions.some(record => smartPayTransactionNeedsReconciliation(record, member.payerId, record.refId));
        return <div key={member.id}><span>{member.displayName} · {member.email}{" "}<small>{zh ? "付款人 ID" : "PayerID"} {member.payerId || "—"}</small></span>{needsReconciliation ? <button type="button" onClick={() => void manualSync(member)} disabled={Boolean(busy)}>{busy === `sync:${member.id}` ? "…" : (zh ? "手动核对此会员" : "Reconcile this member")}</button> : null}</div>;
      })}</div> : null}
    </section>
