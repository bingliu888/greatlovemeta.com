...
function ClassCompanionCameraStage({client,lang,localTrack}:{client:RTKClient;lang:Lang;localTrack?:MediaStreamTrack}){const[revision,setRevision]=useState(0);useEffect(()=>{const timer=window.setInterval(()=>setRevision(value=>value+1),500);return()=>window.clearInterval(timer)},[]);const cameras=useMemo(()=>{void revision;const items:Array<{id:string;name:string;track:MediaStreamTrack;local:boolean}>=[];const selfTrack=localTrack||client.self.videoTrack;if(client.self.videoEnabled&&selfTrack)items.push({id:"self",name:client.self.name||"You",track:selfTrack,local:true});const peers=new Map<string,ScreenPeer>();for(const map of[client.participants.joined,client.participants.active,client.participants.videoSubscribed])for(const peer of map.toArray()as unknown as ScreenPeer[])if(/ · (?:Screen|Camera)$/u.test(peer.name||""))peers.set(peer.id,peer);const ids=[...peers.keys()];if(ids.length)void client.participants.subscribe(ids,["video"]).catch(()=>undefined);for(const peer of peers.values())if(peer.videoEnabled&&peer.videoTrack)items.push({id:peer.id,name:peer.name||"Participant",track:peer.videoTrack,local:false});return items},[client,localTrack,revision]);if(!cameras.length)return null;return <section className="class-screen-share-stage" aria-label={lang==="zh"?"共享摄像头":"Shared camera"}><header><strong>{lang==="zh"?"正在共享摄像头":"Camera sharing"}</strong><span>{cameras.length}</span></header>{cameras.map(item=><CompanionCameraPlayer key={item.id} item={item}/>)}</section>}
