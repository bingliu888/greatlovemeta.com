...
export function NotificationBar() {
  const router = useRouter(); const pathname = usePathname(); const zh = pathname.startsWith("/zh");
  const lang = zh ? "zh" : "en"; const [notice, setNotice] = useState<Notice | null>(null);
  useEffect(() => { let active = true; const poll = async () => { try { const response = await fetch("/api/messages?notifications=1", { cache: "no-store" }); if (!response.ok) return; const data = await response.json() as { notifications?: Notice[] }; if (active && !notice) setNotice(data.notifications?.[0] || null); } catch {} }; void poll(); const timer = window.setInterval(poll, 2500); return () => { active = false; window.clearInterval(timer); }; }, [notice]);
