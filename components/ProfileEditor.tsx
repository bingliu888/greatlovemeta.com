...
export function ProfileEditor({ lang, email, initialName, initialIntroducer, initialImageUrl = "" }: { lang: "en" | "zh"; email: string; initialName: string; initialIntroducer: Introducer; initialImageUrl?: string }) {
  const zh = lang === "zh";
  ...
  async function save(event: React.FormEvent) {
    ...
    setMessage("");
    try {
      const response = await fetch("/api/profile", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ displayName, preferredLanguage: lang }) });
