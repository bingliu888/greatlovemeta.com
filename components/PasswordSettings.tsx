...
function readableError(issue: unknown, zh: boolean) {
  if (isClerkAPIResponseError(issue)) {
    ...
    return error?.longMessage || error?.message || (zh ? "无法保存密码，请重试。" : "Could not save your password. Please try again.");
  }
  return issue instanceof Error ? issue.message : (zh ? "无法保存密码，请重试。" : "Could not save your password. Please try again.");
}
