...
export function ClassPlaylistManager({
  ...
  const load = useCallback(async () => {
    const response = await fetch(`/api/classes/${code}/playlist`, {
      cache: "no-store",
    });
  ...
  async function upload(file: File) {
    ...
    try {
      ...
      for (
        ...
        const partResponse = await fetch(`/api/classes/${code}/playlist`, {
          method: "POST",
          headers: {
      ...
      const done = await fetch(`/api/classes/${code}/playlist`, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
      ...
      if (init?.key && init.uploadId)
        void fetch(`/api/classes/${code}/playlist`, {
          method: "POST",
          headers: { "content-type": "application/json" },
          body: JSON.stringify({
            action: "abort-upload",
  ...
  async function remove(item: PlaylistItem) {
    ...
    const response = await fetch(
      `/api/classes/${code}/playlist?id=${encodeURIComponent(item.id)}`,
      { method: "DELETE" },
    );
    setBusy(false);
    if (response.ok) {
      ...
      {loading ? (
        ...
        <ol className="class-playlist-list">
          {items.map((item, index) => (
            <li key={item.id}>
              <video
                src={`/api/classes/${code}/playlist/${item.id}#t=0.1`}
                muted
                playsInline
                preload="metadata"
              />
