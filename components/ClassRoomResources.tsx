...
export function ClassRoomResources({code,lang,manager,roomTabId,selfStreaming,panel,onClose,onLocalNoteBusyChange,
  ...
  const load=useCallback(async()=>{
    const [files,audio]=await Promise.all([fetch(`/api/classes/${code}/materials`,{cache:"no-store"}),fetch(`/api/classes/${code}/audio-notes`,{cache:"no-store"})]);
    ...
    setMaterials(f.materials||[]);setNotes(a.items||[]);setCanAdd(a.canAdd);setMessage("");
  },[code,zh]);
  useEffect(()=>{
    if(!panel)return;
    let stopped=false,timer:number|undefined,failures=0;
    const poll=async()=>{
      try{await load();failures=0;}
      catch(error){failures++;if(!stopped)setMessage(error instanceof Error?error.message:String(error));}
      finally{if(!stopped){const base=document.hidden?30000:5000;
        timer=window.setTimeout(()=>void poll(),Math.min(60000,base*2**Math.min(failures,3)));}}
    };
    void poll();
    return()=>{stopped=true;if(timer!==undefined)window.clearTimeout(timer);};
  },[panel,load]);
  async function uploadMaterial(file?:File){if(!file||busy)return false;setBusy(true);setMessage("");try{
    const response=await fetch(`/api/classes/${code}/materials`,{method:"POST",headers:{"content-type":file.type||"application/octet-stream","x-file-name":encodeURIComponent(file.name),"x-file-size":String(file.size)},body:file});
  ...
  async function uploadAudio(blob:Blob,duration:number,source:"browser"|"file"){
    ...
    setBusy(true);setMessage("");try{
      const response=await fetch(`/api/classes/${code}/audio-notes`,{method:"POST",headers:{"content-type":blob.type||"audio/webm","x-recording-seconds":String(duration),"x-recording-source":source,"x-recording-size":String(blob.size),"x-room-tab-id":roomTabId},body:blob});
      if(response.status===402){window.location.assign(`/plans?uiLocale=${lang}`);return;}
  ...
  async function speak(item:Material){if(speaking===item.id){stopSpeech();return;}if(!window.speechSynthesis){setMessage(zh?"浏览器不支持文字朗读":"Speech synthesis unavailable");return;}
    stopSpeech();const token=speechToken.current;try{const response=await fetch(`/api/classes/${code}/materials/${item.id}`,{cache:"no-store"});if(!response.ok)throw new Error("File unavailable");
  ...
  async function remove(kind:"material"|"audio-note",id:string){if(!window.confirm(zh?"删除此文件？":"Delete this file?"))return;
    setBusy(true);try{const route=kind==="material"?"materials":"audio-notes";const response=await fetch(`/api/classes/${code}/${route}/${id}`,{method:"DELETE"});if(!response.ok)throw new Error("Delete failed");await load();}catch(error){setMessage(error instanceof Error?error.message:String(error));}finally{setBusy(false);}}
  ...
  return <aside className="class-room-drawer" role="dialog" aria-label={panel==="recordings"?(zh?"全部录音":"All recordings"):(zh?"全部附件":"All attachments")}>
    <header><h2>{panel==="recordings"?(zh?"全部录音":"All recordings"):(zh?"全部附件":"All attachments")}</h2><div className="class-room-drawer-actions">
      {panel==="recordings"&&manager&&!selfStreaming&&!recording&&!draft&&!editing&&<button type="button" disabled={busy} aria-label={zh?"添加录音":"Add recording"} onClick={()=>{if(!canAdd){window.location.assign(`/plans?uiLocale=${lang}`);return;}setMenu(!menu);}}>＋</button>}
    ...
    <input ref={textInput} hidden type="file" accept=".txt,text/plain" onChange={async event=>{const file=event.target.files?.[0];event.target.value="";if(!file)return;if(file.size>MAX_MEETING_SPEECH_TEXT_BYTES){setMessage(zh?"文件不能超过 20 千字节":"Text file exceeds 20 KB");return;}const value=await file.text();setSpeechText(value.slice(0,5000));}}/>
    ...
    <div className="class-room-drawer-list">
      {panel==="recordings"&&<>
        ...
        {notes.map(item=><article key={item.id}><strong>{new Date(item.createdAt*1000).toLocaleString(lang)}</strong><small>{Math.floor(item.recordingSeconds/60)}:{String(item.recordingSeconds%60).padStart(2,"0")}</small><audio controls preload="none" src={`/api/classes/${code}/audio-notes/${item.id}`}/>{manager&&<button type="button" onClick={()=>void remove("audio-note",item.id)}>{zh?"删除":"Delete"}</button>}</article>)}
      ...
      {panel==="files"&&<>{materials.filter(item=>!meetingSpeechScriptName(item.fileName)).map(item=><article key={item.id}><strong>{item.fileName}</strong><small>{new Date(item.createdAt*1000).toLocaleString(lang)}</small><a href={`/api/classes/${code}/materials/${item.id}`} target="_blank" rel="noreferrer">{zh?"查看":"View"}</a>{manager&&<button type="button" onClick={()=>void remove("material",item.id)}>{zh?"删除":"Delete"}</button>}</article>)}{!materials.length&&<p>{zh?"还没有附件":"No attachments yet"}</p>}</>}
    </div>
  </aside>;
}
