...
import { smartPayAvailablePlans, smartPayCheckoutDisplayAmount, smartPayOptionsForPlan, type SmartPayCheckoutOption } from "../lib/smartpay-checkout";
...
const subscriptionTerm = (months: number, zh: boolean) => months === 12
  ? (zh ? "12 个月订阅" : "12-month subscription")
  : (zh ? "1 个月订阅" : "1-month subscription");

export function CryptoCheckout({ lang: locale, initialPlan }: { lang: SiteLanguage; initialPlan: Plan }) {
  const zh = locale === "zh";
  ...
  useEffect(() => {
    ...
    void Promise.all([
      ...
      if (!active) return;
      setOptionsLoaded(true);
      setMessage(zh ? "暂时无法读取当前链上付款项目。" : "Unable to read the active on-chain payment options right now.");
    });
    return () => { active = false; };
  }, [initialPlan, zh]);
  ...
  const ensureLogin = () => {
    if (!status.signedIn) {
      window.location.assign(`/${locale}/auth/login?returnTo=${encodeURIComponent(`/${locale}/pricing/crypto?plan=${plan}`)}`);
      return false;
    }
    return true;
  };
  ...
  async function connectWallet() {
    ...
    try {
      ...
      const result = await connectEvmWallet({
        ...
        appName: GREATLOVEMETA_WALLET_CONNECT.checkout.appName,
        description: GREATLOVEMETA_WALLET_CONNECT.checkout.description,
  ...
  async function lookupExistingPayment(context: PaymentLookupContext) {
    ...
    const response = await fetch("/api/billing/crypto/find-payment", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ plan, settingId: selectedOption.settingId, includeClaimed: includeClaimedPaymentForLookup(context) })
    });
  ...
  async function reconcileExistingPayment(payment: ExistingPayment) {
    ...
    try {
      if (payment.claimed && payment.currentPeriodEnd) {
        ...
        setMessage(zh
          ? `该付款已经确认，订阅至 ${new Date(payment.currentPeriodEnd * 1000).toLocaleDateString(locale)}。`
          : `This payment is already confirmed. The subscription is active through ${new Date(payment.currentPeriodEnd * 1000).toLocaleDateString(locale)}.`);
        return;
      }
      const response = payment.paymentId
        ? await fetch("/api/billing/crypto/smartpay/claim", {
          method: "POST",
          headers: { "content-type": "application/json" },
          body: JSON.stringify({ settingId: selectedOption.settingId, paymentId: payment.paymentId })
        })
        : null;
      ...
      const result = response?.ok && directResult?.verified
        ...
        : (await verifyCryptoPaymentWithConfirmations({
          ...
          onRetry: ({ retryNumber }) => setMessage(zh
      ...
      setMessage(currentPeriodEnd
        ? (zh
          ? `付款已确认，订阅至 ${new Date(currentPeriodEnd * 1000).toLocaleDateString(locale)}。`
          : `Payment confirmed. The subscription is active through ${new Date(currentPeriodEnd * 1000).toLocaleDateString(locale)}.`)
        : (zh ? "付款已确认，订阅现已生效。" : "Payment confirmed. The subscription is now active."));
      await loadStatus();
    } catch {
      setMessage(zh ? "暂时无法核对已有付款。" : "Unable to reconcile the existing payment right now.");
    } finally {
      setBusy("");
    }
  }
  ...
  async function refreshConnectedPreflight(provider: EthereumProvider, address: string) {
    ...
    try {
      ...
      if (result.preflight.nextAction === "pay" || result.preflight.nextAction !== smartPay5Preflight?.nextAction) setApprovalHash("");
      setMessage(!result.preflight.primaryEnough
          ...
          : !result.preflight.eligibilityMet
            ? (zh ? "钱包 GLC 余额或 10 亿 GLC 门槛不足；不会发送授权或付款交易。" : "The wallet lacks the required GLC balance or 1-billion-GLC threshold. No approval or payment will be sent.")
            : result.preflight.gasEnough === false
              ? (zh ? "钱包 Gas 代币不足；不会发送交易。" : "The wallet does not have enough gas token. No transaction will be sent.")
              : result.usingCombo
                ? (zh ? `钱包 GLC 余额达到门槛，已提供 ${smartPayCheckoutDisplayAmount(prepared, true)} 双币付款方案；付款前会再次确认实际余额。` : `The wallet meets the GLC threshold. The ${smartPayCheckoutDisplayAmount(prepared, true)} two-token offer is available, and the actual balances will be confirmed before payment.`)
                : offer.mode === "single"
  ...
  async function sendPayment() {
    ...
    if (!provider) {
      setMessage(zh ? "未检测到已连接的钱包。" : "No connected wallet was detected.");
      return;
    }
    let submittedHash = "";
    ...
    try {
      ...
      if (previous) {
        ...
        setMessage(zh ? "已找到尚未入账的同项目付款。请先核对该付款，避免重复支付。" : "An unreconciled payment for this option was found. Reconcile it first to avoid paying twice.");
        return;
      }
      ...
      if (!offer) throw new Error("RULE_UNAVAILABLE");
        ...
        if (approvalHash && preflight.nextAction !== "pay") {
          setMessage(zh ? "授权交易已发送，但钱包节点尚未显示新额度。请稍后刷新授权状态，不要重复授权。" : "The approval was submitted, but the wallet node still shows the old allowance. Refresh approval status shortly; do not approve again.");
          return;
        }
        if (preflight.nextAction !== "pay") {
          ...
          setMessage(zh ? `请在钱包确认 ${symbol} 授权。` : `Confirm the ${symbol} approval in your wallet.`);
          ...
          const transition = await waitForSmartPayApprovalTransition({ previousAction: preflight.nextAction, read: () => readSmartPay5PreflightForAmounts(provider, wallet, prepared, primaryRequired, secondaryRequired), onRead: state => setSmartPay5Preflight(state) });
          if (!transition.transitioned) {
            setMessage(zh ? "授权已上链，但钱包节点仍在更新额度。请稍后刷新授权状态，不要重复授权。" : "The approval is on-chain, but the wallet node is still updating the allowance. Refresh approval status shortly; do not approve again.");
            return;
          }
          setApprovalHash("");
          setMessage(transition.state.nextAction === "pay" ? (zh ? "授权已确认。请再次点击付款按钮打开付款确认。" : "Approval confirmed. Click the payment button again to open payment confirmation.") : (zh ? "第一种代币授权已确认。请再次点击以授权第二种代币。" : "The first token approval is confirmed. Click again to approve the second token."));
          return;
        }
        setApprovalHash("");
        setMessage(primaryRequired > 0n && secondaryRequired > 0n
          ? (zh ? "两种代币授权已确认；请在钱包确认双币付款。" : "Both token approvals are ready. Confirm the two-token payment in your wallet.")
        ...
        await waitForTransactionReceipt(provider, hash);
        setMessage(zh ? "付款已上链，正在读取交易记录并更新订阅。" : "The payment is on-chain. Reading the transaction record and updating the subscription.");
      ...
      setMessage(submittedHash
        ? (zh ? `付款请求已发送（${submittedHash.slice(0, 10)}…），但订阅尚未完成核对。请勿重复付款；使用“查找或核对已有付款”继续。` : `The payment was submitted (${submittedHash.slice(0, 10)}…), but subscription reconciliation is still pending. Do not pay again; use “Find or verify an existing payment” to continue.`)
        : reason === "WRONG_CHAIN"
        ? (zh ? "钱包当前网络与所选付款网络不一致，请切换网络后重试。" : "Your wallet is on a different network. Switch to the selected network and retry.")
        : reason === "PAYMENT_LOOKUP_FAILED" || /lookup|查找|读取/i.test(reason)
          ? (zh ? "付款前无法完成已有付款检查，因此没有打开钱包或发送交易。请稍后重试。" : "The existing-payment check could not complete, so the wallet was not opened and no transaction was sent. Retry later.")
        : reason === "RULE_UNAVAILABLE" || reason.includes("not currently enabled")
          ? (zh ? "该付款项目已不在当前链上可用清单中，请返回重新选择。" : "This option is no longer available in the current on-chain list. Go back and choose again.")
          : reason === "INSUFFICIENT_TOKEN_BALANCE"
            ? (zh ? `钱包 ${selectedOption.tokenSymbol} 余额不足，未发送授权或付款交易。` : `The wallet does not have enough ${selectedOption.tokenSymbol}. No approval or payment was sent.`)
            : reason === "INSUFFICIENT_GLC_BALANCE"
              ? (zh ? "钱包 GLC 余额或 10 亿 GLC 门槛不足，未发送授权或付款交易。" : "The wallet lacks the required GLC balance or 1-billion-GLC threshold. No approval or payment was sent.")
            : reason === "INSUFFICIENT_NATIVE_BALANCE"
              ? (zh ? `钱包 ${walletChain(selected).nativeCurrency.symbol} Gas 余额不足，未发送交易。` : `The wallet does not have enough ${walletChain(selected).nativeCurrency.symbol} for gas. No transaction was sent.`)
              : reason === "PAYMENT_SIMULATION_FAILED"
                  ? (zh ? "付款模拟已回滚，未打开钱包确认；这不是 Gas 余额提示。请管理员检查合约状态。" : "The payment simulation reverted, so wallet confirmation was not opened. This is not a gas-balance warning; ask an administrator to inspect the contract.")
                : reason === "WALLET_CHANGED"
                  ? (zh ? "钱包账户已切换。请重新连接后再继续。" : "The wallet account changed. Reconnect before continuing.")
                : submittedApprovalHash
                  ? (zh ? `授权 ${submittedApprovalHash.slice(0, 10)}… 已发送。请刷新授权状态，不要重复授权。` : `Approval ${submittedApprovalHash.slice(0, 10)}… was submitted. Refresh approval status; do not approve again.`)
                : code === 4001
                  ? (zh ? "您已在钱包中取消本次操作。" : "You rejected this wallet action.")
                : code === -32002
                  ? (zh ? "钱包已有待处理请求。请先在钱包中完成或关闭它。" : "The wallet already has a pending request. Complete or close it first.")
                  : (zh ? "交易未发送或未完成。请检查钱包授权、余额和网络。" : "The transaction was not sent or completed. Check wallet approval, balance, and network."));
  ...
  async function verify(candidateHash?: string, afterSuccessfulPayment = false) {
    ...
    if (!/^0x[a-f0-9]{64}$/.test(hash)) {
      ...
      try {
        ...
        setMessage(zh ? "暂时无法查找付款。" : "Unable to look up a payment right now.");
        setBusy("");
        return;
      }
      if (!existing) {
        setMessage(zh ? "尚未找到符合条件的付款。" : "No matching payment was found.");
        setBusy("");
        return;
      }
    ...
    setMessage(zh ? "交易已找到，正在等待链上确认并更新订阅…" : "Transaction found. Waiting for confirmations and updating your subscription…");
    const { data } = await verifyCryptoPaymentWithConfirmations({
      plan,
      settingId: selectedOption.settingId,
      txHash: hash,
      initialDelayMs: afterSuccessfulPayment ? 6_000 : 0,
      onInitialWait: () => setMessage(zh
        ? "付款已确认上链；6 秒后首次读取交易记录。请勿重复付款。"
        : "The payment is confirmed on-chain. The first transaction-record check will run in 6 seconds. Do not pay again."),
      onRetry: ({ retryNumber }) => setMessage(zh
    ...
    if (data.verified) {
      ...
      setMessage(data.currentPeriodEnd
        ? (zh
          ? `交易已确认，订阅至 ${new Date(data.currentPeriodEnd * 1000).toLocaleDateString(locale)}。`
          : `Transaction confirmed. The subscription is active through ${new Date(data.currentPeriodEnd * 1000).toLocaleDateString(locale)}.`)
        : (zh ? "交易记录已验证，订阅现已生效。" : "The transaction record was verified. Your subscription is active."));
      await loadStatus();
    } else {
      setPendingPaymentHash(hash);
      setMessage(zh
        ? `付款 ${hash.slice(0, 10)}… 已发送，但三次延迟核对仍未完成。请勿重复付款；稍后点击“重新核对已发送付款”。`
        : `Payment ${hash.slice(0, 10)}… was submitted, but three delayed reconciliation attempts did not complete. Do not pay again; use “Retry submitted payment reconciliation” later.`);
    }
    setBusy("");
  }
  ...
  return <div className="crypto-flow">
    <Link className="back-link" href={`/${locale}/pricing`}>← {zh ? "返回订阅方案" : "Back to pricing"}</Link>
    <div className="page-heading centered"><p className="eyebrow"><span/> CRYPTO PAYMENT</p><h1>{zh ? "使用加密货币订阅" : "Subscribe with crypto"}</h1><p>{zh ? "可选服务期、代币和金额全部来自本站当前可用的链上付款规则。不会要求私钥或助记词。" : "Terms, tokens, and amounts come only from this site's currently available on-chain payment rules. We never request a private key or seed phrase."}</p></div>
    <ol className="payment-steps"><li className={step >= 1 ? "active" : ""}>1. {zh ? "选择服务期" : "Product"}</li><li className={step >= 2 ? "active" : ""}>2. {zh ? "选择代币" : "Crypto"}</li><li className={step >= 3 ? "active" : ""}>3. {zh ? "付款钱包" : "Wallet"}</li></ol>

    {step === 1 ? <section className="crypto-box flow-card">
      <h2>{zh ? "步骤 1：选择订阅服务期" : "Step 1: Choose a subscription term"}</h2>
      <p className="flow-intro">{zh ? "这里只显示至少有一个可用链上付款项目的服务期。" : "Only terms with at least one available on-chain payment option are shown."}</p>
      {!optionsLoaded ? <p>{zh ? "正在读取链上付款项目…" : "Loading on-chain payment options…"}</p> : availablePlans.length ? <div className="radio-list">{availablePlans.map(item => <label key={item.id}><input type="radio" checked={plan === item.id} onChange={() => choosePlan(item.id)}/><span className="plan-option-copy"><strong>{subscriptionTerm(item.months, zh)}</strong><small>{zh ? "链上可付款" : "Available on-chain"}</small></span><span className="plan-option-price"><b>{item.price}</b><small>{zh ? "有可用付款选项" : "Payment options available"}</small></span></label>)}</div> : <p className="billing-message">{zh ? "当前没有可用的订阅付款项目。请等待管理员确认链上付款规则。" : "There is no available subscription payment option right now. Wait for an administrator to confirm the on-chain payment rules."}</p>}
      <button className="button primary" disabled={!availablePlans.length} onClick={() => setStep(2)}>{zh ? "下一步：选择代币" : "Next: choose crypto"} →</button>
    </section> : null}

    {step === 2 ? <section className="crypto-box flow-card">
      <button className="text-button" onClick={() => setStep(1)}>← {zh ? "上一步" : "Back"}</button>
      ...
      {planOptions.length ? <div className="radio-list">{planOptions.map(option => {
        return <label key={option.key}><input type="radio" checked={selectedOption?.key === option.key} onChange={() => { setSettingId(option.settingId); setSmartPay5Preflight(null); setApprovalHash(""); setSmartPay5UsingCombo(false); setExistingPayment(null); setPendingPaymentHash(""); setConfirmedUntil(null); }}/><span className="plan-option-copy"><strong>{option.tokenSymbol}</strong><small>{option.chainName}</small></span><span className="plan-option-price"><b>{smartPayCheckoutDisplayAmount(option)}</b><small>{zh ? "链上原价" : "On-chain price"}</small></span></label>;
    ...
    {step === 3 && selected && selectedOption ? <section className="crypto-box flow-card">
      ...
      {!direct ? <>
        {connected ? <div className="direct-payment">
          <p className="connected-wallet-line" style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}><span style={{ minWidth: 0, overflowWrap: "anywhere" }}>{zh ? "已连接钱包" : "Connected wallet"} · {wallet}</span><button type="button" className="wallet-refresh-button" style={{ width: 32, height: 32, flex: "0 0 32px", display: "grid", placeItems: "center", padding: 0, border: "1px solid rgba(18,32,42,.18)", borderRadius: 999, background: "#fff", color: "inherit", cursor: "pointer" }} disabled={preflightBusy || Boolean(busy)} onClick={() => walletProvider && void refreshConnectedPreflight(walletProvider, wallet)} aria-label={approvalHash ? (zh ? "刷新授权状态" : "Refresh approval status") : (zh ? "刷新余额与 Gas" : "Refresh balances and gas")} title={approvalHash ? (zh ? "刷新授权状态" : "Refresh approval status") : (zh ? "刷新余额与 Gas" : "Refresh balances and gas")}><svg viewBox="0 0 24 24" aria-hidden="true" style={{ width: 16, height: 16, fill: "none", stroke: "currentColor", strokeWidth: 2, strokeLinecap: "round", strokeLinejoin: "round" }}><path d="M20 11a8 8 0 1 0-2.34 5.66M20 4v7h-7"/></svg></button></p>
          ...
          {existingPayment ? <div className={`existing-crypto-payment${existingPayment.claimed ? " confirmed" : ""}`} role="status">
            <strong>{existingPayment.claimed ? (zh ? "已找到已确认付款" : "Confirmed payment found") : (zh ? "已找到尚未入账付款" : "Unreconciled payment found")}</strong>
            <span>{existingPayment.timestamp ? new Date(existingPayment.timestamp * 1000).toLocaleString(locale) : (zh ? "链上付款" : "On-chain payment")} · {existingPayment.txHash.slice(0, 12)}…</span>
            {existingPayment.currentPeriodEnd ? <small>{zh ? `当前订阅至 ${new Date(existingPayment.currentPeriodEnd * 1000).toLocaleDateString(locale)}` : `Current subscription through ${new Date(existingPayment.currentPeriodEnd * 1000).toLocaleDateString(locale)}`}</small> : null}
            {existingPaymentAction(existingPayment.claimed) === "reconcile" ? <button type="button" className="button primary" disabled={Boolean(busy)} onClick={() => void reconcileExistingPayment(existingPayment)}>{busy === "verify" ? "…" : (zh ? "核对尚未入账付款" : "Reconcile unreconciled payment")}</button> : null}
          </div> : pendingPaymentHash ? <div className="existing-crypto-payment pending" role="status">
            <strong>{zh ? "付款已发送，等待核对" : "Payment submitted; reconciliation pending"}</strong>
            <span>{pendingPaymentHash.slice(0, 12)}…</span>
            <small>{zh ? "请勿重复付款。系统已按 10 秒间隔重试三次。" : "Do not pay again. The system already retried three times at 10-second intervals."}</small>
            <button type="button" className="button primary" disabled={Boolean(busy)} onClick={() => void verify(pendingPaymentHash)}>{busy === "verify" ? "…" : (zh ? "重新核对已发送付款" : "Retry submitted payment reconciliation")}</button>
          </div> : confirmedUntil ? <div className="existing-crypto-payment confirmed" role="status"><strong>{zh ? "付款与订阅已确认" : "Payment and subscription confirmed"}</strong><small>{zh ? `订阅至 ${new Date(confirmedUntil * 1000).toLocaleDateString(locale)}` : `Subscription through ${new Date(confirmedUntil * 1000).toLocaleDateString(locale)}`}</small></div> : <button className="button primary" disabled={Boolean(busy) || preflightBusy || Boolean(approvalHash)} onClick={() => void sendPayment()}>{busy === "send" ? "…" : approvalHash ? (zh ? "请先刷新授权状态" : "Refresh approval status first") : smartPay5Preflight?.nextAction === "approve-primary" ? (zh ? `授权 ${smartPay5Offer?.primaryTokenSymbol || "代币"}` : `Approve ${smartPay5Offer?.primaryTokenSymbol || "token"}`) : smartPay5Preflight?.nextAction === "approve-secondary" ? (zh ? `授权 ${smartPay5Offer?.secondaryTokenSymbol || "代币"}` : `Approve ${smartPay5Offer?.secondaryTokenSymbol || "token"}`) : (zh ? "立即付款" : "Pay now")}</button>}
