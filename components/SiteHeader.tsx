...
const labels = {
  en: { members: "Members", community: "Community", academy: "BingAcademy", claw: "MyClaw", real: "WhatsReal" },
  zh: { members: "会员", community: "社区", academy: "BingAcademy", claw: "MyClaw", real: "WhatsReal" },
};

function GlobalLinks({ lang }: { lang: "en" | "zh" }) {
  const copy = labels[lang];
  return <><Link href={`/${lang}/members`}>{copy.members}</Link><Link href={`/${lang}/community`}>{copy.community}</Link><a href="https://bingacademy.com">{copy.academy}</a><a href="https://myclaw.one">{copy.claw}</a><a href="https://whatsreal.com">{copy.real}</a></>;
}
