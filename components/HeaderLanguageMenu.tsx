...
export function HeaderLanguageMenu({ lang, mobile = false, onNavigate }: { lang: SiteLanguage; mobile?: boolean; onNavigate?: () => void }) {
  ...
  if (mobile) return <details ref={menuRef} className="mobile-language-menu"><summary aria-label={t.chooseLanguage}><GlobeIcon/></summary><div className="header-language-options"><strong>{t.language}</strong>{options}</div></details>;
