...
CREATE INDEX IF NOT EXISTS class_rooms_type_start_idx ON class_rooms(class_type,status,starts_at);
CREATE INDEX IF NOT EXISTS class_rooms_host_updated_idx ON class_rooms(host_user_id,updated_at DESC);
...
CREATE INDEX IF NOT EXISTS class_invites_email_idx ON class_invites(email,created_at DESC);
...
CREATE INDEX IF NOT EXISTS class_chat_room_created_idx ON class_chat_messages(room_id,created_at);

CREATE TABLE IF NOT EXISTS class_media_presence (
  id TEXT PRIMARY KEY NOT NULL,
  room_id TEXT NOT NULL REFERENCES class_rooms(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS class_presence_active_idx ON class_media_presence(room_id,active,last_seen_at);
...
CREATE INDEX IF NOT EXISTS class_materials_room_created_idx ON class_materials(room_id,created_at DESC);
