...
export default async function AuthPage({ params, searchParams }: { params: Promise<{ lang: string; mode: string }>; searchParams: Promise<{ returnTo?: string }> }) {
  ...
  const brand = lang === "zh" ? "大爱元宇宙" : lang === "zh-tw" ? "大愛元宇宙" : "GreatLoveMeta.com";
  return <main className="auth-page"><aside className="auth-art gc-auth-art"><Link className="brand inverse" href={`/${lang}`}><span className="brand-seal gc-brand-seal">GL</span><span>{brand}</span></Link><div className="gc-auth-mark" aria-hidden="true"><span>GL</span><i/><i/><i/></div><blockquote>{t.quote}</blockquote><p>{t.meta}</p></aside><section className="auth-panel"><div className="auth-top"><Link href={`/${lang}`}>← {t.back}</Link><LanguageLink lang={lang}/></div><div className="auth-box"><p className="eyebrow">{t.eyebrow}</p><h1>{t.title}</h1><AuthPolicyIntro locale={lang}/><ClerkAuthForm lang={lang} returnTo={returnTo}/></div></section></main>;
}
