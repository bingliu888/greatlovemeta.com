...
const content = {
  en: {
    ...
    playGame: "Play",
    playNote: "Sign in for Play · 1 saved session per game daily",
  ...
  zh: {
    ...
    communityLink: "加入社区",
    communityMeta: "论坛 · 实时聊天 · 会员交流",
    appTitle: "下载大爱应用",
    appBody: "通过官方 Android 或 iOS 版本，在手机上继续使用大爱生态。",
    android: "Android 安装包下载",
    ios: "App Store 下载",
    ...
    accessCommunityBody: "加入论坛、认识会员，并使用群组实时聊天。",
    ...
    gamesBody: "无需账户即可试玩；登录后开始正式游戏，每局完成成绩会自动加入您的个人每日游戏记录。",
    gameCards: [
      { key: "monopoly", tag: "幸运挑战", title: "幸运轮盘", body: "转动璀璨轮盘，在三次机会中累积成绩，看看幸运最终落在哪里。" },
      { key: "miner", tag: "深空街机", title: "星际矿工", body: "每局发射 3 次，命中获得 10,000–120,000 GLC，未命中为 0；每天保存 1 局正式成绩。" },
    ],
    testTrial: "试玩",
    playGame: "正式游戏",
    playNote: "正式游戏需要登录 · 每款游戏每天最多保存 1 局",
    swapKicker: "大爱兑换",
    swapTitle: "链上兑换",
    ...
    membersBody: "浏览公开成员档案而不暴露私人邮箱，并通过社区与实时聊天建立连接。",
    membersCta: "查看会员",
...
export default async function LanguageHome({ params }: { params: Promise<{ lang: string }> }) {
  ...
  return <main>
    ...
    <section className="community-section"><div className="community-card"><div><p className="section-kicker">{t.communityKicker}</p><h2>{t.communityTitle}</h2><p>{t.communityBody}</p><a className="primary-button" href={`/${lang}/community`}>{t.communityCta}<span>→</span></a><Link data-testid="home-community-classes" className="secondary-button" href={`/${lang}/classes`}>{lang === "zh" ? "进入课堂" : "Enter Classes"} →</Link><Link className="secondary-button" href={`/${lang}/classes?view=mine`}>{lang === "zh" ? "我的课程" : "My Classes"} →</Link></div><div className="four-seats gc-network" aria-hidden="true"><span>{lang === "zh" ? "成员" : "MEMBERS"}</span><span>{lang === "zh" ? "助手" : "GURU"}</span><strong>{lang === "zh" ? "您" : "YOU"}</strong><span>{lang === "zh" ? "聊天" : "CHAT"}</span><span>{lang === "zh" ? "共建" : "BUILD"}</span></div></div></section>
