...
export default async function LanguageHome({ params }: { params: Promise<{ lang: string }> }) {
  ...
  return <main>
    ...
    <section className="community-section"><div className="community-card"><div><p className="section-kicker">{t.communityKicker}</p><h2>{t.communityTitle}</h2><p>{t.communityBody}</p><a className="primary-button" href={`/${lang}/community`}>{t.communityCta}<span>→</span></a><Link data-testid="home-community-classes" className="secondary-button" href={`/${lang}/classes`}>{lang === "zh" ? "进入课堂" : "Enter Classes"} →</Link></div><div className="four-seats gc-network" aria-hidden="true"><span>{lang === "zh" ? "成员" : "MEMBERS"}</span><span>{lang === "zh" ? "助手" : "GURU"}</span><strong>{lang === "zh" ? "您" : "YOU"}</strong><span>{lang === "zh" ? "聊天" : "CHAT"}</span><span>{lang === "zh" ? "共建" : "BUILD"}</span></div></div></section>
