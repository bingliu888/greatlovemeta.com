...
const content = {
  ...
  zh: {
    ...
    testTrial: "试玩",
    playGame: "Play",
    playNote: "Play 需要登录 · 每款游戏每天最多保存 1 局",
    swapKicker: "大爱兑换",
    swapTitle: "链上兑换",
    ...
    <section className="glm-games-section" id="games">
      ...
      <div className="glm-game-grid">
        {t.gameCards.map((game, index) => <article className={`glm-game-card ${game.key}`} key={game.key}>
          ...
          <div className="glm-game-copy"><small>{String(index + 1).padStart(2, "0")} · {game.tag}</small><h3>{game.title}</h3><p>{game.body}</p><div className="glm-game-actions"><a href={`/games/${game.key}.html?mode=trial&lang=${lang}`}>{t.testTrial}</a><a className="primary" href={`/${lang}/games/${game.key}?mode=play`}>{t.playGame} →</a></div><em>{t.playNote}</em></div>
        </article>)}
      </div>
    </section>
