...
export async function POST(request: Request) {
  ...
  const response = await fetch("https://api.openai.com/v1/responses", { method: "POST", headers: { authorization: `Bearer ${apiKey}`, "content-type": "application/json", "OpenAI-Safety-Identifier": await sha256(`greatlovemeta-public:${visitor}`) }, body: JSON.stringify({ model: process.env.OPENAI_TEXT_MODEL || "gpt-5-nano", instructions: `You are Guru, the bilingual public assistant for GreatLoveMeta.com. Answer in ${language === "zh" ? "Simplified Chinese" : "English"}. Be clear, neutral, concise, and practical. Help with the GreatLove ecosystem, AI agents, RWA concepts, NFTs, membership, Community, Live Chat, events, projects and site guidance. Distinguish facts from opinions, identify uncertainty, and never invent credentials, project status, legal rights, or event details.`, input: messages.join("\n"), reasoning: { effort: "minimal" }, text: { verbosity: "low" }, max_output_tokens: 1200 }) });
