...
export async function POST(request: Request) {
  ...
  if (input.type === "topic") { const title = input.title?.trim(); if (!title || title.length > 100) return NextResponse.json({ error: "Invalid title" }, { status: 400 }); const category = ["general", "identity", "projects", "events"].includes(input.category || "") ? input.category : "general"; const id = createId(); await db.prepare("INSERT INTO community_topics (id, user_id, category, title, body, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)").bind(id, user.id, category, title, body, now, now).run(); return NextResponse.json({ id }); }
