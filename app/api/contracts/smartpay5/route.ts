...
export async function GET(request: Request) {
  ...
  if (!Number.isInteger(chainId) || chainId <= 0 || !isAddress(contractAddress)
    ...
    return NextResponse.json({ error: "Select a published SmartPay5 source file" }, { status: 400 });
  }
  ...
  if (!publication) return NextResponse.json({ error: "SmartPay5 source is not public for this contract" }, { status: 404 });
  if (!publication.sourceCode || !publication.standardJsonInput) {
    return NextResponse.json({ error: "SmartPay5 source artifact is unavailable" }, { status: 503 });
  }
  ...
  return new Response(body, {
    headers: {
      ...
      "content-disposition": `attachment; filename="SmartPay5-${chainId}-${contractAddress}.${extension}"`,
