...
export async function GET(request: Request) {
  ...
  return Response.json({ profile: { displayName: user.displayName, preferredLanguage: user.preferredLanguage, imageUrl: avatar ? `/api/profile?avatar=${encodeURIComponent(user.id)}` : "" }, introducer: introducer ?? null });
}
...
export async function POST(request: Request) {
  ...
  const payload = await request.json() as { displayName?: string; preferredLanguage?: string };
  const displayName = payload.displayName?.trim().slice(0, 60);
  if (!displayName || displayName.length < 2) return Response.json({ error: "Display name is required" }, { status: 400 });
  ...
  return Response.json({ profile: { displayName, preferredLanguage } });
}
