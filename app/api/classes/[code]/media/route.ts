...
export async function GET(
  ...
  await db
    .prepare(
      "UPDATE class_media_presence SET active=0,mic_on=0,camera_on=0 WHERE room_id=? AND last_seen_at<=?",
    )
    .bind(room.id, now - 45)
    .run();
  const activePresence = await db
    .prepare(
      "SELECT 1 AS active FROM class_media_presence WHERE room_id=? AND active=1 AND last_seen_at>? LIMIT 1",
    )
    .bind(room.id, now - 45)
    .first<{ active: number }>();
  ...
  const rawUsers = users.results || [],
    screenShareActive = rawUsers.some(
      (item) =>
        item.identity === "screenshare:" + room.id &&
        Boolean(item.cameraOn) &&
        item.lastSeenAt > now - 15,
    ),
    ...
    managerIds = new Set([
      ...
      ...(
        (
          await db
            .prepare("SELECT user_id AS userId FROM class_cohosts WHERE room_id=?")
            .bind(room.id)
            .run<{ userId: string }>()
        ).results || []
  ...
  const requests =
    access.manager && room.realtimeMode === "webinar"
      ? (
          await db
            .prepare(
              "SELECT identity,display_name AS displayName,media_kind AS mediaKind,status FROM class_stage_requests WHERE room_id=? AND status='pending' ORDER BY created_at LIMIT 100",
            )
            .bind(room.id)
            .run()
        ).results || []
      : [];
  const speakers =
    access.manager && room.realtimeMode === "livestream"
      ? (
          await db
            .prepare(
              "SELECT member_email AS email FROM class_stage_speakers WHERE room_id=? ORDER BY created_at",
            )
            .bind(room.id)
            .run()
        ).results || []
  ...
  if (!canPublish && room.realtimeMode === "livestream" && user)
    canPublish = Boolean(
      await db
        .prepare(
          "SELECT 1 FROM class_stage_speakers WHERE room_id=? AND lower(member_email)=lower(?)",
        )
...
export async function POST(
  ...
  if (body.action === "screen-share") {
    ...
    if (room.streamingMode !== "audio" || room.realtimeMode === "livestream")
      return Response.json(
        { error: "Audio broadcasts cannot share screens" },
        { status: 409 },
      );
    if (body.value)
      await db
        .prepare(
          "INSERT INTO class_media_presence(id,room_id,identity,user_id,display_name,is_member,mic_on,camera_on,active,last_seen_at) VALUES(?,?,?,?,?,0,0,1,1,?) ON CONFLICT(room_id,identity) DO UPDATE SET camera_on=1,active=1,last_seen_at=excluded.last_seen_at",
        )
        .bind(
          createId(),
          room.id,
      ...
      await db
        .prepare(
          "UPDATE class_media_presence SET camera_on=0,active=0,last_seen_at=? WHERE room_id=? AND identity=?",
        )
        .bind(now, room.id, "screenshare:" + room.id)
        .run();
    return Response.json({ screenShareActive: Boolean(body.value) });
  }
  if (body.action === "request-stage") {
    if (room.realtimeMode !== "webinar" || !identity)
    ...
    if (!presence)
      return Response.json(
        { error: "Join as a viewer first" },
        { status: 409 },
      );
    const kind = body.mediaKind === "video" ? "video" : "audio";
    await db
      .prepare(
        "INSERT INTO class_stage_requests(id,room_id,identity,user_id,display_name,media_kind,status,created_at,updated_at) VALUES(?,?,?,?,?,?,'pending',?,?) ON CONFLICT(room_id,identity,media_kind) DO UPDATE SET status='pending',updated_at=excluded.updated_at",
      )
      .bind(
        createId(),
        room.id,
  ...
  if (body.action === "review-stage") {
    ...
    await db
      .prepare(
        "UPDATE class_stage_requests SET status=?,updated_at=? WHERE room_id=? AND identity=? AND media_kind=?",
      )
      .bind(
        ...
        room.id,
        identity,
        body.mediaKind === "video" ? "video" : "audio",
      )
      .run();
    return Response.json({ ok: true });
  }
  if (body.action === "add-speaker" || body.action === "remove-speaker") {
    ...
    if (!/^\S+@\S+\.\S+$/.test(email))
      return Response.json(
        { error: "Enter a valid member email" },
        { status: 400 },
      );
    if (body.action === "add-speaker") {
      ...
      await db
        .prepare(
          "INSERT OR IGNORE INTO class_stage_speakers(id,room_id,member_email,added_by_user_id,created_at) VALUES(?,?,?,?,?)",
        )
        .bind(createId(), room.id, email, user!.id, now)
        .run();
    } else
      await db
        .prepare(
          "DELETE FROM class_stage_speakers WHERE room_id=? AND lower(member_email)=lower(?)",
        )
        .bind(room.id, email)
        .run();
    return Response.json({ ok: true });
  ...
  if (body.action === "heartbeat") {
    await db
      .prepare(
        "UPDATE class_media_presence SET last_seen_at=?,active=1 WHERE room_id=? AND identity=?",
      )
  ...
  if (body.action === "media") {
    ...
    if (!allowed && room.realtimeMode === "webinar")
      allowed = Boolean(
        await db
          .prepare(
            "SELECT 1 FROM class_stage_requests WHERE room_id=? AND identity=? AND status='approved' AND media_kind IN (?,?) LIMIT 1",
          )
          .bind(
            room.id,
            identity,
            body.mic ? "audio" : "video",
            body.camera ? "video" : "audio",
          )
          .first(),
      );
    if (!allowed && room.realtimeMode === "livestream" && user)
      allowed = Boolean(
        await db
          .prepare(
            "SELECT 1 FROM class_stage_speakers WHERE room_id=? AND lower(member_email)=lower(?)",
    ...
    if (!allowed && (body.mic || body.camera))
      return Response.json({ error: "STAGE_ACCESS_REQUIRED" }, { status: 403 });
    if (room.realtimeMode !== "group_call" && (body.mic || body.camera)) {
      const count = await db
        .prepare(
          "SELECT COUNT(*) AS count FROM class_media_presence WHERE room_id=? AND identity<>? AND active=1 AND (mic_on=1 OR camera_on=1) AND last_seen_at>?",
        )
        .bind(room.id, identity, now - 45)
        .first<{ count: number }>();
      if (Number(count?.count || 0) >= 9)
        return Response.json(
          { error: "The 9-speaker stage is full" },
          { status: 409 },
        );
    }
    await db
