...
export async function POST(request:Request,{params}:{params:Promise<{code:string}>}){
  try{
    ...
    const playlistRelay=Boolean(body.playlistRelay&&room.realtimeMode!=="group_call"&&await db.prepare("SELECT 1 FROM class_playlist_state s JOIN class_playlist_items i ON i.room_id=s.room_id WHERE s.room_id=? AND s.active=1 LIMIT 1").bind(room.id).first());
    let providerMeetingId=room.providerMeetingId;
    if((body.start||body.publish||playlistRelay)&&(access.manager||playlistRelay)&&!room.streamActive){const created=await createClassProviderRoom(room.title);providerMeetingId=created.id;await db.prepare("UPDATE class_rooms SET provider_meeting_id=?,stream_active=1,mute_all=0,updated_at=? WHERE id=?").bind(providerMeetingId,now,room.id).run();}
    if(!providerMeetingId||(!room.streamActive&&!body.start&&!body.publish&&!playlistRelay))return Response.json({error:"STREAM_NOT_ACTIVE"},{status:409});
