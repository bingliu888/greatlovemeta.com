...
export async function GET(request: Request) {
  ...
  const normalizedThreads = threads.map(thread => ({ ...thread, preview: visibleBody(String(thread.preview || "")) })); const unread = normalizedThreads.reduce((total, item) => total + Number(item.unread || 0), 0); if (url.searchParams.get("summary") === "1") return NextResponse.json({ unread });
  ...
  const avatars = await avatarsById(); const members = memberRows.map(member => ({ ...member, imageUrl: avatars.get(String(member.id)) || "", online: Number(member.lastSeenAt) >= now - 45 }));
  const teamCount = await db.prepare("SELECT COUNT(*) AS value FROM referrals r JOIN referral_codes c ON c.id = r.referral_code_id WHERE c.user_id = ?").bind(user.id).first<{ value: number }>();
...
export async function POST(request: Request) {
  ...
  if (kind === "team") { const rows = (await db.prepare("SELECT r.referred_user_id AS id FROM referrals r JOIN referral_codes c ON c.id = r.referral_code_id WHERE c.user_id = ?").bind(user.id).run<{ id: string }>()).results || []; recipients = rows.map(row => row.id); if (!recipients.length) return NextResponse.json({ error: "Your referral team is empty" }, { status: 400 }); }
