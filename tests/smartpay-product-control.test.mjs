...
test("SmartPay5 payment items fail closed and Stop remains database-only", async () => {
  ...
  const [admin, route, checkout, optionsRoute, prepareRoute, migration] = await Promise.all([read("components/SmartPayAdminConsole.tsx"), read("app/api/admin/crypto-payments/rules/route.ts"), read("lib/smartpay-checkout-server.ts"), read("app/api/billing/crypto/smartpay/options/route.ts"), read("app/api/billing/crypto/smartpay/prepare/route.ts"), read("drizzle/0126_smartpay5_payment_item_states.sql")]);
  assert.match(admin, /stopSmartPay5Preset/);
  assert.doesNotMatch(admin, /sendOwnerTransaction\(contractState\?\.paused \? "unpause" : "pause"/);
  assert.match(route, /smartPay5RulePresetStatus\(preset, rules\)\.state !== "configured"/);
  assert.match(route, /action === "confirm"/);
  assert.match(route, /\["stop", "confirm"\]\.includes\(action\)/);
  assert.match(route, /stopped: true, selectable: false/);
  assert.match(checkout, /smartPay5PaymentItemDatabaseState/);
  assert.match(checkout, /smartPay5EnabledPresets\(presets, state\.enabledPresetKeys\)/);
  assert.doesNotMatch(checkout, /smartPay5PaymentRules|verifySmartPay5Identity|smartPay5PayoutConfigurationRaw/);
  assert.doesNotMatch(optionsRoute, /smartpay5_payment_item_states|enabledPresetKeys/);
  assert.doesNotMatch(prepareRoute, /smartpay5_payment_item_states|enabledPresetKeys/);
  const db = new DatabaseSync(":memory:");
  db.exec(migration);
  assert.equal(db.prepare("SELECT COUNT(*) AS count FROM smartpay5_payment_item_states").get().count, 0);
  db.close();
});
